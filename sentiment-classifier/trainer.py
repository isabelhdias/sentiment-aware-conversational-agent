...
from pytorch_lightning.callbacks import (
    EarlyStopping,
    #LearningRateLogger,
    LearningRateMonitor, # TODO: confirm. not sure what changed
    ModelCheckpoint,
)
...
def build_trainer(hparams: Namespace) -> pl.Trainer:
    ...
    tb_logger = TensorBoardLogger(
        ...
        version= hparams.experiment_name + "_" + hparams.dataset + "_" + datetime.now().strftime("%d-%m-%Y--%H-%M-%S"),
        name="",
    )
    ...
    trainer = pl.Trainer(
        ...
        # early_stop_callback=early_stop_callback, # do I need this?
        ...
        # log_save_interval=hparams.log_save_interval, # deprecated row_log_interval?
        log_every_n_steps=hparams.log_every_n_steps, # also deprecated. not sure if i need to substitute it
        # distributed_backend="dp",
        precision=hparams.precision,
        weights_summary="top",
        profiler=hparams.profiler,
        num_sanity_val_steps=5,
        stochastic_weight_avg=True, # try to fix seeds problem 
    )
    return trainer
