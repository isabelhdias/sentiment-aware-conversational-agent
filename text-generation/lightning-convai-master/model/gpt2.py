...
class PersonaGPT2(pl.LightningModule):
    ...
    def __init__(self, hparams: Namespace):
        ...
        self.gpt2 = GPT2DoubleHeadsModel.from_pretrained(self.config.pretrained_model)
    ...
    def training_step(
        ...
        loss_val = (
            output.loss * self.config.lm_coef + output.mc_loss * self.config.mc_coef
        )
    ...
    def validation_step(
        ...
        output = {"val_loss": loss_val, "val_nll": nll, "val_acc": acc, "perplexity": torch.exp(nll)}
        return output
    ...
    def validation_epoch_end(
        ...
        self.log("val_perplexity", metrics["val_perplexity"], prog_bar=True)
    ...
    def test_step(
        ...
        output = {"test_loss": loss_val, "test_nll": nll, "test_acc": acc, "perplexity": torch.exp(nll)}
        return output
    ...
    def test_epoch_end(
        ...
        metrics = {
            ...
            "test_perplexity": torch.stack([x["perplexity"] for x in outputs]).mean(),
        }
        self.log("test_loss", metrics["test_loss"], prog_bar=True)
        self.log("test_nll", metrics["test_nll"], prog_bar=True)
        self.log("test_acc", metrics["test_acc"], prog_bar=True)
        self.log("test_perplexity", metrics["test_perplexity"], prog_bar=True)
        self.log("test_perplexity_avg", torch.exp(metrics["test_nll"]), prog_bar=True)
    ...
    def from_experiment(cls, experiment_folder: str):
        ...
        model = cls.load_from_checkpoint(
            checkpoint_path, hparams=Namespace(**hparams), strict=False
        )
