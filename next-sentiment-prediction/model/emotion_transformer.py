...
class EmotionTransformer(pl.LightningModule):
    ...
    class ModelConfig(Config):
        ...
        sentiment_representation: str = "sum"  
        sentiment_embeddings_size: int = 128  

        # Classification
        classification_embeddings: bool = False       
    ...
    def __init__(self, hparams: Namespace):
        ...
        if self.config.classification_embeddings:
            self.classification_embeddings = nn.Embedding(len(self.label_encoder) + 1, 64)
    ...
    def init_sentiment_representations(self, train_dataloader):
        ...
        for label_idx in range(len(self.label_encoder)):
            ...
            mask = gold_labels == label_idx
            # embeddings mean for the label
            label_embedding = torch.mean(embeddings[mask], dim=0)

            # fill token_type_embeddings matrix
            #self.transformer.embeddings.token_type_embeddings.weight[label_idx].data.copy_(label_embedding)
    ...
    def set_classification_head(self):
        ...
        if self.config.classification_embeddings:
            in_features += 64 * self.config.context_turns
    ...
    def configure_optimizers(self):
        ...
        if self.config.classification_embeddings:
            classif_parameters = [
                {
                    "params": self.classification_embeddings.parameters(),
                    "lr": self.config.learning_rate
                }
            ]
    ...
    def forward(
        ...
        classif_labels: torch.Tensor = None,
        step="train",
        ...
        **kwargs,
        ...
        if self.config.classification_embeddings:
            classification_embeddings = self.classification_embeddings(classif_labels)
        ...
        if self.config.retrieval_augmentation and not return_only_embed:
            ...
            # in concat method first we retrieval the sentiment embedding for each example in the batch and
            # redimension it
            if self.config.sentiment_representation == "concat":
                sentiment_embeds = self.sentiment_embeddings(labels_nn)
                sentiment_embeds = self.redimension_embeddings(sentiment_embeds)

            # in sum method we retrieve the sentiment embedding for each token of each example in the batch
            # and sum it to the word embeddings
            elif self.config.sentiment_representation == "sum":
                ...
                # sum word embeddings with sentiment embeddings
                inputs_embeds = inputs_embeds + sentiment_embeds
            
            # if input_embeds are passed the model doesn't recalculate them
            ...
            output = self.transformer(
                input_ids=input_ids, 
                attention_mask=mask, 
                token_type_ids=nn_sentiment_ids,
                output_hidden_states=True,
            )
        ...
        # unpack values from different models
        if len(output) == 3:
        ...
        # Classify. If concat, we concatenate the sentemb to the sentiment_embeds
        if self.config.retrieval_augmentation: #and self.config.sentiment_representation == "concat":
            return self.classification_head(torch.cat((sentemb, sentiment_embeds), dim=1))
        
        if self.config.classification_embeddings:
            try:
                result = self.classification_head(torch.cat((sentemb, classification_embeddings.view(classification_embeddings.shape[0], -1)), dim=1))
            except:
                import pdb; pdb.set_trace()
            return result

        # otherwise regular classification
    ...
    def training_step(
        ...
        elif self.config.classification_embeddings:
            input_ids, input_lengths, labels, classif_labels = batch
            logits = self.forward(
                input_ids=input_ids, 
                input_lengths=input_lengths,
    ...
    def validation_epoch_end(
        ...
        if self.config.dataset == "goemotions":
            ...
            self.log("macro-precision", metrics["macro-precision"].to(self.transformer.device))
            self.log("macro-recall", metrics["macro-recall"].to(self.transformer.device))
            ...
            self.log("micro-f1-no-mc", micro_f1_no_majority_class, prog_bar=True)
        return {
    ...
    def test_epoch_end(
        ...
        self.log('y_hat_predictions', y_hat_predictions)
