...
def read_nn(dataset_path):
    ...
    with open(dataset_path + "nsp_train_nn_quorum_5.json") as train_file:
        labels_nn['train'] = json.load(train_file)
    
    with open(dataset_path + "nsp_valid_nn_quorum_5.json") as valid_file:
        labels_nn['valid'] = json.load(valid_file)

    with open(dataset_path + "nsp_test_nn_quorum_5.json") as test_file:
...
def evaluate(dataset, labels_nn, label_encoder, set='train'):
    ...
    for i in tqdm(range(len(dataset) - 2), desc="reading dataset"):
        if i != 0:
            if dataset[i]['conv_id'] == dataset[i-1]['conv_id']: #dataset[i]['conv_id'] == dataset[i+1]['conv_id'] and dataset[i]['conv_id'] == dataset[i+2]['conv_id']:
                y.append(int(dataset[i]['label']))
